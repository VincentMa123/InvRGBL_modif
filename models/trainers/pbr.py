...
def image_space_pbr(albedo_map, normal_map, roughness_map, metallic_map, sunvis_map,
    ...
    if env_diffuse_mode == "neutral":
        env_diffuse = torch.ones_like(env_diffuse) * 0.5
    ...
    # Apply env split scales and ambient floor
    env_diffuse = env_diffuse * float(env_diffuse_scale) + float(env_ambient_floor)
    env_specular = env_specular * float(env_specular_scale)
