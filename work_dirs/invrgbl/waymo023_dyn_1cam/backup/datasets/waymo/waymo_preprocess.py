...
class WaymoProcessor(object):
    ...
    def convert_one(self, file_idx):
        ...
        for frame_idx, data in enumerate(
            ...
            if "intensity" in self.process_keys:
                self.save_intensity(frame, file_idx, frame_idx)
    ...
    def save_intensity(self, frame, file_idx, frame_idx):
        """Parse and save the LiDAR intensity maps projected to camera images.
        ...
        for cam_id in range(5):
            calib = frame.context.camera_calibrations[cam_id]
            projected = project_vehicle_to_image(frame.pose, calib, points)
            u = projected[:, 0].astype(int)
            v = projected[:, 1].astype(int)
            ok = projected[:, 2]

            height, width = calib.height, calib.width
            intensity_map = np.zeros((height, width), dtype=np.float32)

            valid = ok == 1
            u_valid = np.clip(u[valid], 0, width - 1)
            v_valid = np.clip(v[valid], 0, height - 1)
            intensity_valid = intensity[valid]

            intensity_map[v_valid, u_valid] = intensity_valid

            intensity_path = f"{self.save_dir}/{str(file_idx).zfill(3)}/intensity/{str(frame_idx).zfill(3)}_{cam_id}.npy"
            np.save(intensity_path, intensity_map[..., None])
    ...
    def create_folder(self):
        ...
        for i in id_list:
            ...
            if "intensity" in self.process_keys:
                os.makedirs(f"{self.save_dir}/{str(i).zfill(3)}/intensity", exist_ok=True)
            if "dynamic_masks" in self.process_keys:
